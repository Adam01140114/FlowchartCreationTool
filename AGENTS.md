...
| Public site (Render, `https://formwiz.onrender.com`) | `node server.js`: the dev server in public mode (`FORMWIZ_PUBLIC=1`). It answers only the published forms (`/form/<site name or projectId>/...`), `POST /edit_pdf`, the read-only flowchart view the forms' "View flowchart" button opens (`/flowchart/<project>`), and a list of the forms at `/` - everything else is 404 (`publicAllows` in `dev-server.js`). Render deploys `main` and runs `node server.js`; `npm start` is the private server with everything. A site's `live-site.json` keeps the `stamp` its links carry, worked out on the machine that published it: worked out again on Render's UTC clock, a 9:12pm build read `saved=9-15-26_4-12am` |
