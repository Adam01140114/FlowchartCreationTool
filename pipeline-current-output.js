...
async function main() {
  ...
  for (const source of sources.sort()) {
    ...
    if (fs.existsSync(filledPdf)) {
      // The document itself, beside the pictures of it.
      //
      // A published run should be one directory that holds everything the
      // dashboard needs. It used to point View PDF at pipeline-out/, which is
      // scratch - regenerated by the next fill, and not something to keep - so
      // the link worked on the machine that produced it and nowhere else.
      fs.copyFileSync(filledPdf, path.join(target, 'filled.pdf'));
    ...
    manifest.forms.push({ name: folderNameFor(base), pages: pages.length, fields: counts,
      pdf: fs.existsSync(filledPdf) ? 'filled.pdf' : null });
