...
| `FormWiz GUI/generate.js` | **Form HTML generator** — question styles, section cards, debug menu, nav, test/prod deployment UI, PDF fill helpers. In test mode: right-click a question → **Edit question** (the words only - title, choices, box labels, an entry's title; ids, option values and PDF boxes never change; kept per project in localStorage `fwTextEdits_<projectId>`, put back on load and after every change, since a count draws its entries later); debug menu → **View flowchart** (`/flowchart/<project>`, read-only) and **Export flowchart** (the project's flowchart from `/api/project/<project>` with those words written in: a title, box label or entry title into the question cell's `_questionText`/`value`, `_textboxes` labels, `_placeholder` and `_dropdownTitle`; a choice's new words into the project's top-level **`displayWords`**, keyed `option:<select id>:<value>` or `checkbox:<input id>`). A choice's words cannot go into its option cell: the option's text *is* its value and the root of its field names (`library.js` makes a checkbox option's nameId from its text), so renaming the cell renamed the field and broke the PDF fill. `displayWords` travels project → `window.projectDisplayWords` (`project-forms.js`) → GUI JSON `displayWords` (`project-gui-export.js`) → `window.displayWordsConfig` (`download.js`, both ways) → the page's `var fwDisplayWords` (`generate.js`), which `fwApplyDisplayWords` draws over the values in every deployment. A project rebuilt from its spec by `pipeline-build-packet.js` has no `displayWords`. The editor offers only words the flowchart holds: a dropdown's options when the select carries `data-fw-choice-list` and a checkbox when its input carries `data-fw-choice` (both set where the generator draws option nodes - a state list, a number list and "None of the above" have neither), and a box label or entry title only when the project has those words. A question whose words several flowchart nodes share (a packet asks it once for every form) is renamed in all of them, so the packet still asks it once; two questions on the page that read the same are told apart by their field names. After a rebuild from the exported flowchart, an edit the page already reads is dropped from the browser |
